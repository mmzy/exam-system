  ...
  <body onload="sT()">
    <table align="center" width="1003" height="485" border="0" cellpadding="0" cellspacing="0" class="centerbg">
    	<tr>
    		...
    		<form onsubmit="ajaxSubmitForm(this, true)" action="<%=path %>/exam/score" method="post">
    			<table width="98%" border="0" align="center" cellpadding="0" cellspacing="10">
		    				...
		    				<c:if test="${model.textModelId == textmodel.id }">
			    				...
			    				<c:forEach  items="${examList }" var="exam" varStatus="vs">
			    					<c:if test="${exam.text.texId == textmodel.id }">
					    				...
					    				<c:if test="${exam.text.texId != 1 and exam.text.texId != 2 }">
						    				...
						    				<c:choose  >
						    				   <c:when test="${texts.texId == 7}">
							    				<tr>				
							    						<td colspan="3"><textarea rows="5" cols="270" name="examList[${vs.count }].answer">${exam.answer }</textarea></td>
							    				</tr>
							    				</c:when>
							    				<c:otherwise>
							    					<tr>				
					    				...
					    				<tr>
				    						<td colspan="3">
				    							<span style="color:red;font-size:16px;">正确答案：
				    							
				    							<c:choose  >
						    				   <c:when test="${exam.text.texId == 7}">
							    							
							    					<textarea rows="5" cols="270" >${exam.text.answer }</textarea>
				    							...
				    							<c:if test="${exam.text.answer eq exam.answer }">
				    								<input type="text" style="width:25px;" name="examhisAnswerList[${vs.count }].grade" value="${model.grade }">
				    							</c:if>
				    							<c:if test="${exam.text.answer ne exam.answer }">
				    								<input type="text" style="width:25px;" name="examhisAnswerList[${vs.count }].grade" value="">
				    							</c:if>
				    						</td>
					    				</tr>
					    			</c:if>
    				...
    				<tr>
    					<td colspan="3">
    						<div align="center">
    							<input type="submit" value="批阅试卷" name="submit">
    						</div>
    					</td>
    				</tr>
