        ...
        <tbody>
            <c:forEach items="${publishexamList.list }" var="publishExam">
                <tr>
                    ...
                    <td>
                    	<button class="btn size-MINI radius" onclick="showLayer('未批阅试卷','<%=path%>/exam/examStudent?id=${publishExam.id }')">未批阅试卷</button>
                    	<button class="btn size-MINI radius" onclick="showLayer('已批阅试卷','<%=path%>/exam/examStudent2?id=${publishExam.id }')">已批阅试卷</button>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
