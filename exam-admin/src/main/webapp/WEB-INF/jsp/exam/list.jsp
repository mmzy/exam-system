    ...
    <table class="table table-border table-bordered table-bg table-hover">
        <thead>
            <tr>
                ...
                <th>考试类型</th>
                <th>发布人</th>
                <th>操作 <button class="btn size-M radius" onclick="showLayer('发布考试','<%=path%>/exam/add')"> 发布考试</button></th>
            </tr>
        </thead>
        <tbody>
            <c:forEach items="${publishexamList.list }" var="publishExam">
                <tr>
                    ...
                    <td>
                        <button class="btn size-MINI radius" onclick="ajaxDelete('<%=path%>/exam/delete', 'id=${publishExam.id}')">删除</button>
                        <button class="btn size-MINI radius" onclick="showLayer('修改考试信息','<%=path%>/exam/update?id=${publishExam.id }')">修改</button>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
