...
<body>
<div class="pd-20">
	<form action="<%=path%>/classesStudent/update">
        ...
        <table class="table table-border table-bordered table-bg table-hover">
           <thead>
                <tr>
                    ...
                    <th>手机</th>
                    <th>邮箱</th>
                    <th>操作 <button class="btn size-M radius" onclick="showLayer('添加新成员','<%=path%>/classesStudent/add?id=${id }')"> 添加</button></th>
                </tr>
            </thead>
            <tbody>
            	<c:forEach items="${pageInfo.list }" var="student">
                    <tr>
                        ...
                        <td>${student.email }</td>
                        <td>
                            <button class="btn size-MINI radius" onclick="ajaxSubmit('<%=path%>/classesStudent/delete', 'id=${student.id }')">从班级移除</button>
                        </td>
                    </tr>
                  </c:forEach>
