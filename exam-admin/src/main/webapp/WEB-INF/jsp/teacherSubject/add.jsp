...
<body>
<div class="pd-20">
	<form action="<%=path%>/teacherSubject/add">
            ...
            <tbody>
            	<c:forEach items="${pageInfo.list }" var="subject">
                    <tr>
                        <td>${subject.name }</td>
                        <td>
                            <button class="btn size-MINI radius" onclick="ajaxSubmit('<%=path%>/teacherSubject/add', 'id=${subject.id}&tea_id=${id }')">为老师添加所授科目</button>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
