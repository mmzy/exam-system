...
<div class="pd-20">
    ...
    <table class="table table-border table-bordered table-bg table-hover">
        <thead>
            <tr>
                <th>学科名称</th>
                <th>操作 <button class="btn size-M radius" onclick="showLayer('添加学科','<%=path%>/teacherSubject/add?id=${id }')"> 添加</button></th>
            </tr>
        </thead>
        <tbody>
        	<c:forEach items="${pageInfo.list }" var="subject">
	        	<tr>
	                <td>${subject.name }</td>
	                <td>
	                    <button class="btn size-MINI radius" onclick="ajaxDelete('<%=path%>/teacherSubject/delete', 'id=${subject.id}&tea_id=${id }')">移除</button>
	                </td>
	            </tr>
            </c:forEach>    
