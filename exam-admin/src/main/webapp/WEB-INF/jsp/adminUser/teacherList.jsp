        ...
        <tbody>
        	<c:forEach items="${pageInfo.list }" var="teacher">
                <tr>
                    ...
                    <td>${teacher.name }</td>
                    <td>
                        <button class="btn size-MINI radius" onclick="ajaxDelete('<%=path%>/adminUser/deleteTeacher', 'id=${teacher.id}')">删除</button>
                        <button class="btn size-MINI radius" onclick="ajaxSubmit('<%=path%>/adminUser/resetTeacherPassword', 'id=${teacher.id}')">重置密码</button>
                    </td>
                </tr>
             </c:forEach> 
        </tbody>
