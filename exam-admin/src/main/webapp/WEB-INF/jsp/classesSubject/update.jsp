...
<body>
<div class="pd-20">
	<form action="<%=path%>/teacherSubject/update">
    ...
    <table class="table table-border table-bordered table-bg table-hover">
        <thead>
            <tr>
                <th>学科名称</th>
                <th>任课老师</th>
                <th>操作 <button class="btn size-M radius" onclick="showLayer('添加学科','<%=path%>/classesSubject/add?classesId=${classesId }')"> 添加</button></th>
            </tr>
        </thead>
        <tbody>
        
        	<c:forEach items="${pageInfo.list }" var="teacherSubject">
	        	<tr>
	                <td>${teacherSubject.subjectName }</td>
	                <td>${teacherSubject.teacherName }</td>
	                <td>
	                    <button class="btn size-MINI radius" onclick="ajaxSubmit('<%=path%>/classesSubject/delete', 'subId=${teacherSubject.subjectId }&classesId=${classesId }&teacherId=${teacherSubject.teacherId }')">从班级移除</button>
	                </td>
	            </tr>
             </c:forEach>
        </tbody>
